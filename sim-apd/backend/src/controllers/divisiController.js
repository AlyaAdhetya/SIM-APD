...
const getListDivisi = async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM divisi_apd ORDER BY nama_divisi ASC');
...
const saveDivisi = async (req, res) => {
  try {
    ...
    const [existing] = await db.query('SELECT * FROM divisi_apd WHERE nama_divisi = ?', [nama_divisi]);

    if (existing.length > 0) {
      // Update
      await db.query('UPDATE divisi_apd SET wajib_apd = ? WHERE id = ?', [wajib_apd ? 1 : 0, existing[0].id]);
      return jsonSuccess(res, null, 'Berhasil memperbarui divisi.');
    } else {
      // Insert
      const [result] = await db.query('INSERT INTO divisi_apd (nama_divisi, wajib_apd) VALUES (?, ?) RETURNING id', [nama_divisi, wajib_apd ? 1 : 0]);
